//falta (borrar las clases de los inputs despues de agregar el alumno) o (volver a agregar el submit a los botones y ver como deshabilitar uno de los botones mientras esta el otro o eliminar los eventos)
//corregir evento del input dni una vez que modifico el alumno desde el formulario
...
// recibe un objeto y una condicion a evaluar, y le agrega o quita la clase para validar o invalidar el input
...
function validarDni () {
    ...
    if (parseInt(dni) >= 0 && buscarAlumnoDni(dni) == -1){        
        test = true;                
    }  
    agregarClase (this, test);
    validarBotonAgregar();
...
//busca si existe un alumno con ese dni en el localStorage, si lo encuentra retorna la posicion, si no -1
...
//comprueba que en el formulario haya 3 inputs con la clase "is-valid", ya que tanto el campo nombre, dni y email tienen que ser validos
function validarBotonAgregar () {    
    ...
    if (inputs.length === 3){
//------------------------deberia tomar la variable global------------------        
...
//------------------------------------------------------------------------------------
//Adaptar HTML para agregar una o mas notas a alumnos ya cargados.
//------------------------------------------------------------------------------------
...
function agregarNotas(alumno){
    ...
    '<li class="list-group-item">'+
        '<div class="row"><div class="col-11"><h5 class="text-center">Notas</h5></div><button class="btn btn-danger btn-sm col" onclick="cerrar()">X</button></div>'+
        '<li class="list-group-item"><h6 class="text-center">'+alumno.nombre + ' ' + alumno.apellido + ' (DNI: '+alumno.dni+')</h6></li>'+
    '</li>';
    var liNotas = document.createElement("li");
    //list-group-item
    liNotas.className = "input-group";
    var vectorNotas = alumno.notas;
    for (var i = 0; i < vectorNotas.length; i++) {     
        liNotas.innerHTML += '<input type="number" class="form-control form-control-sm text-center" value="'+vectorNotas[i]+'">';
        //para que haya un maximo de 3 notas por fila
        if (liNotas.getElementsByTagName("input").length == 3){
            listaNotas.appendChild(liNotas);
            liNotas = document.createElement("li");
            liNotas.className = "input-group";
        }
    }
    listaNotas.appendChild(liNotas);   
    listaNotas.innerHTML += 
        '<li class="list-group-item"><div class="row"><div class="col-6"><input class="form-control form-control" type="number" placeholder="Nueva nota"></div>'+
...
function actualizarNotas(alumno){
    ...
    for (i=0; i < inputs.length; i++){
        var numero = parseInt(inputs[i].value);
        //agrego la nota solo cuando es mayor a 0
        //si el input esta vacio, el parseInt me daria un NaN, pero NaN >= 0 es falso y tmp entraria
...
function buscarAlumnoNombre () {    
    ...
    //si existe un elemento con clase active en la lista, se la remuevo
...
function validarModificarDni () { 
    ...
    var dni = document.getElementsByClassName("active")[0].id;
...
function modificarAlumno () {
    ...
    var inputApellido = document.getElementById("inputApellido").value.trim();
    ...
    vectorAlumnos[posicion] = new crearAlumno(inputNombre, inputApellido, inputDni.trim(), inputEmail, notas);
    //agrego el vector al localStorage
    setLocalList("alumnosLocal", vectorAlumnos);
    //muestro el boron agregar
    var botonAgregarAlumno = document.getElementById("agregarAlumno");
    ...
    resetearFormulario(formulario);
//------------------------------------------------------------------------------------------------        
    //agrego y remuevo los eventos para que vuelva a invalidar el input cuando el dni existe
    document.getElementById("inputDni").removeEventListener("keyup", validarModificarDni);
    document.getElementById("inputDni").addEventListener("keyup", validarDni);
//------------------------------------------------------------------------------------------------        
